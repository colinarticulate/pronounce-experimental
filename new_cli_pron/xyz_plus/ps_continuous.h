    ...
    public:
        ...
        int retrieve_results(void){
            ...
            if (cmd_ln_boolean_r(_config, "-backtrace")) {
                ...
                const char *hyp = _ps_get_hyp(_ps, &score);
                ...
                if (hyp != NULL) {
                    ...
                    for ( seg = _ps_seg_iter(_ps); seg; seg = _ps_seg_next(seg) ) {
                        int sf, ef;
                        char const *word = _ps_seg_word(seg);
                        _ps_seg_frames(seg, &sf, &ef);
        ...
        char const *
        _ps_get_hyp(ps_decoder_t *ps, int32 *out_best_score)
        {
            char const *hyp;
        ...
        ps_seg_t *
        _ps_seg_next(ps_seg_t *seg)
        {
            return ps_search_seg_next(seg);
        }

        char const *
        _ps_seg_word(ps_seg_t *seg)
        {
            return seg->word;
        }

        void
        _ps_seg_frames(ps_seg_t *seg, int *out_sf, int *out_ef)
        {
            int uf;
