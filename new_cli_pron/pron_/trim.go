...
func fetchTrimBounds(audiofile string, phons []phoneme) (float64, float64) {
	...
	// _debug("\n length of audio = ", length_audio)
	// if err != nil {
	// 	_debug("Call to mt_cv_mvns failed with err, ", err)
	// }
...
func webRtcBounds(audiofile string, mode int) (float64, float64) {
	...
	if err != nil {
		_debug("webRtcBounds: call to wav.NewReader failed. err, info.Size, file, audiofile =", err, info.Size(), file, audiofile)
		log.Panic(err)
	}
